...
const Login = () => {
    ...
    useEffect(() => {
        ...
        // Retry if script isn't immediately parsed
    ...
    const handleGoogleLoginSuccess = async (response) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setMessage(data.error || 'Google login verification failed.');
          ...
          <div className='login-content signup-container flex h-screen'>
            <div className='signup-form w-1/2 flex flex-col items-center justify-center p-6 sm:p-12'>
                ...
                {screenState === 'username-selection' ? (
                    ...
                    <div className="w-full max-w-sm text-center animate-fadeIn">
                        <h1 className='text-lg font-bold text-white mb-6'>Login to your account</h1>

                        {/* Google Sign-in as Primary Option */}
                        <div className="flex flex-col items-center justify-center gap-4 mb-8">
                            <div id="google-signin-btn" className="shadow-lg hover:scale-102 transition duration-200"></div>
                            
                            <button 
                                onClick={() => setShowTraditional(!showTraditional)} 
                                className="text-zinc-500 hover:text-white text-xs underline cursor-pointer mt-2"
                            >
                                {showTraditional ? 'Hide email options' : 'Or login with email and password'}
                            </button>
                        </div>
                        ...
                        {showTraditional && (
                            <form onSubmit={handleSubmit} className="animate-slideDown">
                                <div className='signupt-input-group flex flex-col gap-4'>
                                    <fieldset className="fieldset bg-black/10 border-[#1db9546f] hover:border-[#1db954cb] rounded-box border p-4 text-left">
                                        <legend className="fieldset-legend text-zinc-400">Traditional login</legend>
                        ...
                        <p className="text-zinc-500 text-xs mt-6">
                            Don't have an account? <a href="/signup" className="text-[#1db954] hover:underline font-bold ml-1">Sign Up</a>
                        </p>
                    </div>
                )}
            </div>
